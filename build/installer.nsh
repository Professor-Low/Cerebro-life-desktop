...
!macro customInstall
  ...
  FileWrite $R0 "    }$\r$\n"
  FileWrite $R0 "    # Verify exclusions were actually applied (Tamper Protection can silently block)$\r$\n"
  FileWrite $R0 "    $$p = Get-MpPreference$\r$\n"
  FileWrite $R0 "    $$hasPath = $$p.ExclusionPath -contains '$INSTDIR'$\r$\n"
  FileWrite $R0 "    $$hasProc = $$p.ExclusionProcess -contains 'Cerebro.exe'$\r$\n"
  FileWrite $R0 "    $$marker = Join-Path $$env:USERPROFILE '.cerebro\.defender-excluded'$\r$\n"
  FileWrite $R0 "    if ($$hasPath -and $$hasProc) {$\r$\n"
  FileWrite $R0 "        Set-Content -Path $$marker -Value (Get-Date -Format o) -Force$\r$\n"
  FileWrite $R0 "        Set-Content -Path '$INSTDIR\.defender-excluded' -Value (Get-Date -Format o) -Force$\r$\n"
  FileWrite $R0 "    } else {$\r$\n"
